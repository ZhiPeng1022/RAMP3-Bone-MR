# FinnGen 骨折临床结局三角验证：
# forearm fracture (finn-b-ST19_FRACT_FOREA)
# lumbar spine/pelvis fracture (finn-b-ST19_FRACT_LUMBAR_SPINE_PELVIS)
# 暴露：GTEx top5 + eQTLGen RAMP3
...
out <- extract_outcome_data(
  snps = snps,
  outcomes = c("finn-b-ST19_FRACT_FOREA", "finn-b-ST19_FRACT_LUMBAR_SPINE_PELVIS")
)
cat("FinnGen 骨折匹配行数：", nrow(out), "\n")
write.csv(out, file.path(DATA, "finn_fracture_outcomes.csv"), row.names = FALSE)
...
for (oid in c("finn-b-ST19_FRACT_FOREA", "finn-b-ST19_FRACT_LUMBAR_SPINE_PELVIS")) {
  ...
  if (nrow(m1) >= 2) {
    ...
    result_lines <- c(result_lines, sprintf("- GTEx top5: IVW logOR=%.5f, SE=%.5f, P=%.3g, n=%d",
                                            r["beta"], r["se"], r["p"], r["n"]))
  } else {
  ...
  if (nrow(m2) >= 2) {
    ...
    result_lines <- c(result_lines, sprintf("- eQTLGen: IVW logOR=%.5f, SE=%.5f, P=%.3g, n=%d",
                                            r["beta"], r["se"], r["p"], r["n"]))
  } else {
...
out_md <- c(
  "# FinnGen 骨折临床结局三角验证",
  "",
  "> 暴露 = GTEx v10 top5 RAMP3 eQTL 与 eQTLGen RAMP3 全血 eQTL；结局为 log OR（骨折风险）",
  result_lines
)
writeLines(out_md, "results/summary/finn_fracture_results.md")
