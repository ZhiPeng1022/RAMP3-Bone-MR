# 骨转换标志物 MR：RAMP3 eQTL -> Osteocalcin pQTL / 血清碱性磷酸酶
...
out <- extract_outcome_data(
  snps = snps,
  outcomes = c("prot-a-246", "ebi-a-GCST90025947", "ebi-a-GCST90013991")
)
cat("标志物匹配行数：", nrow(out), "\n")
write.csv(out, file.path(DATA, "bone_marker_outcomes.csv"), row.names = FALSE)
...
for (oid in unique(out$id.outcome)) {
  ...
  if (nrow(m1) >= 2) {
    ...
    result_lines <- c(result_lines, sprintf("- GTEx top5: beta=%.5f, SE=%.5f, P=%.3g, n=%d",
                                            r["beta"], r["se"], r["p"], r["n"]))
  }
  ...
  if (nrow(m2) >= 2) {
    ...
    result_lines <- c(result_lines, sprintf("- eQTLGen: beta=%.5f, SE=%.5f, P=%.3g, n=%d",
                                            r["beta"], r["se"], r["p"], r["n"]))
  }
}

writeLines(c("# 骨转换标志物 MR 结果", "",
             "> 暴露 = RAMP3 eQTL（GTEx top5 / eQTLGen）", result_lines),
           file.path(DATA, "bone_marker_results.md"))
