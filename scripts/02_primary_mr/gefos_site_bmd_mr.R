# GEFOS Zheng 2015 部位 BMD 独立复制：
# femoral neck (ieu-a-980), forearm (ieu-a-977), lumbar spine (ieu-a-982)
# 暴露：GTEx v10 top5 + eQTLGen RAMP3
...
out <- extract_outcome_data(
  snps = snps,
  outcomes = c("ieu-a-977", "ieu-a-980", "ieu-a-982")
)
cat("部位 BMD 总匹配行数：", nrow(out), "\n")
write.csv(out, file.path(DATA, "gefos_site_bmd_outcomes.csv"), row.names = FALSE)
...
for (oid in c("ieu-a-977", "ieu-a-980", "ieu-a-982")) {
...
out_md <- c(
  "# GEFOS 部位 BMD 独立复制结果",
  "",
  "> Zheng 2015；暴露 = GTEx v10 top5 RAMP3 eQTL 与 eQTLGen RAMP3 全血 eQTL",
  result_lines
)
writeLines(out_md, "results/summary/gefos_site_bmd_results.md")
