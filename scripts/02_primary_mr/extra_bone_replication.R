# 新增独立骨结局：
# Barton 2021 heel BMD T-score (ebi-a-GCST90025982, n=445,855)
# FinnGen osteoporosis (finn-b-G6_OSTEOPOROSIS)
...
out <- extract_outcome_data(
  snps = snps,
  outcomes = c("ebi-a-GCST90025982", "finn-b-G6_OSTEOPOROSIS")
)
cat("额外骨结局匹配行数：", nrow(out), "\n")
write.csv(out, file.path(DATA, "extra_bone_outcomes.csv"), row.names = FALSE)
...
for (oid in c("ebi-a-GCST90025982", "finn-b-G6_OSTEOPOROSIS")) {
...
writeLines(out_md, "results/summary/extra_bone_replication_results.md")
